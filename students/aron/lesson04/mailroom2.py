...
def main():
    ...
    main_response = 0
    while main_response != actions.index('Quit') +1:
        valad_response = False
        while valad_response != True:
            main_response = int(input(build_menu(actions)))
            if main_response <= (len(actions) +1):
                valad_response = True
            if main_response == actions.index('Add new donation or new donor.')+1:
            ...
            if main_response == actions.index('Send letters to all donors.')+1:
                send_email_to_all()
            if main_response == actions.index('Create a Report.') +1:  #generate_report():
                generate_report()
