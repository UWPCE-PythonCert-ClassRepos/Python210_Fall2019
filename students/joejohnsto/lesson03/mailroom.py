...
def thank_you():
    ...
    #add some way to let user quit back to main menu at any time (maybe call main? - if response = quit: main())
    response = 'list'
    ...
    while response == 'list':
        ...
        if response == 'list':
            ...
            check = input(f'Is "{response}" the correct donor name?\n \
                          Please respond "y" or "n"')
            if check != 'y':
                response = 'list'
    # add donor to database if not already there   
    name = response.title()
    if not [x for x, y in donor_db if x == name]:
        donor_db.append((name, []))
    # get donation amount
    amount = float(input(f'Enter the amount that {name} donated: '))
    # add new donation to database under donor name
    idx = [x for x, y in donor_db].index(name)
    donor_db[idx][1].append(amount)
    ...
    print(ty_note.format(name, amount))
