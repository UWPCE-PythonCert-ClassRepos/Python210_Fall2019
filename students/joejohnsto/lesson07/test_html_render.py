...
def test_kwargs():
    page = Html()
    page.append("some plain text.")
    page.append(P("A simple paragraph of text", style="background-color:rgb(255, 99, 71);"))
    d = {'class': 'intro'}
    p2 = P("Some more plain text.", **d)
    page.append(p2)
    file_contents = render_result(page)
    print(file_contents)
    assert 'style="background-color:rgb(255, 99, 71);"' in file_contents
    assert type(page.attributes) == dict
    assert type(p2.attributes) == dict
