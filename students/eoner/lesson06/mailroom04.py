...
def create_report():
    ...
    # for k, v in donors.items(): #use keys?
        ...
        # print(f'{k:20} {total:{17}.2f} {numGifts:15} {AvgGift:{20}.2f}')
    return[(f'{k:20} {sum(v):{17}.2f} {len(v):15} {(sum(v)/len(v)):{20}.2f}') for k,v in donors.items()]
...
def thanks_all():
    #more comprehension
    return [create_thankYou(k,v[-1]) for k, v in donors.items()]


def create_thankYou(name, last_Donation):
    #create letter
    output = TY_LETTER.format(name,last_Donation)
    #call file oeration to write the file
