...
def test_init():
    ...
    e = Element("this is some text", id="test")
...
def test_render_element():
    ...
    assert file_contents.startswith("<abstract_tag>")
    assert file_contents.endswith("</abstract_tag>")

    # make sure 'html' tags occur once
    assert file_contents.count('<abstract_tag>') == 1
    assert file_contents.count('</abstract_tag>') == 1
...
def test_one_line_tag_append():
    ...
    """
    e = OneLineTag("this is one line")
    with pytest.raises(NotImplementedError):
        e.append("this is another line")
...
def test_attributes():
    ...
    # but make sure the embedded elements tags get rendered!
    # first test the end tag is there -- same as always
    assert file_contents.endswith("</p>")

    # but now the opening tag is far more complex
    # but it starts the same:
    assert file_contents.startswith("<p ") # make sure there's no space after the p

    # order of tags is not important in html, so we need to
    # make sure not to test for that
    # but each attribute should be there
    assert 'style="text-align: center"' in file_contents
    assert 'id="intro"' in file_contents

    # just to be sure there should be a closeing bracket to the opening tag
...
def test_hr_attr():
    """A simple horizontal rull with attributes"""
    hr = Hr(width=400)
    file_contents = render_result(hr)
    print(file_contents)
...
def test_ul():
    ul = Ul("this is some text")
    ul.append("and this is some more text")
...
def test_ul_attr():
    """A simple unordered list rule with attributes"""
...
def test_h():
    h = H(2, "this is some content")
    file_contents = render_result(h).strip()
    print(file_contents)
    assert file_contents == '<h2>this is some content</h2>'
...
#    lines = file_contents.split('\n')
#    # making sure the opening and closing tags are right.
#    assert lines[0] == "<abstract tag>"
#    # this line should be indented by the amount specified
#    # by the class attribute: "indent"
#    assert lines[1].startswith(Element.indent + "thi")
#    assert lines[2] == "</abstract tag>"
#    assert file_contents.endswith("</abstract tag>")
